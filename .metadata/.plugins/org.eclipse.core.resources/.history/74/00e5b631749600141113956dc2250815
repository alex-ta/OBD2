...
public class BluetoothStream {
	...
	public BluetoothStream(BluetoothSocket socket) throws SocketError{
		 ...
		 * der Socket enthält das Verbunde Gerät, falls kein Grät verbunden ist, wird ein
		 * SocketError ausgelöst, da offensichtlich kein Stream ohne zweites Gerät etabliert
		 * werden kann. Das verbunde Gerät wir zusätzlich im Attribute device gespeichert, falls
	...
	public byte[] read(){
		/**
		 * Diese Methode prüft die Verbindung der Geräte,
		 * wenn diese Verbunden sind, werden die eingehenden Daten ausgelesen
		 * und als Bytes zurückgibt.
		 * */
		if(!isConnected()){
	...
	public boolean write (String sqz){
		/**
		 * Diese Methode prüft die Verbindung der Geräte,
		 * wenn diese Verbunden sind, werden die Übergebenen Daten
		 * als Bytes zum anderen Geräte übertragen.
		 * */
		if(!isConnected()){
	...
	private boolean isConnected(){
		/**
		 * Diese Methode prüft die Verbindung,
		 * falls keine Verbindung vorhanden ist, gibt 
		 * wird einaml versucht sich mit dem gespeicherten
		 * Gerät zu verbinden. Das Ergebnis der Verbindung wird 
		 * durch einen Boolean zurückgegen (true für verbudnen).
		 * */
